...
class RespuestaMapper
{
    ...
    {
        $stmt = $this->db->prepare("insert into respuesta(id_respuesta, id_foro, id_pregunta, texto, fecha, likes_pos, likes_neg, id_usuario) value (?,?,?,?,?,?,?,?)");
        $stmt->execute(array($respuesta->getIdRespuesta(), $respuesta->getIdForo(), $respuesta->getIdPregunta(), $respuesta->getTexto(), $respuesta->getFecha(),
    ...
    public function listarRespuestasPorPregunta($pag = 1, $id_pregunta)
    {
        ...
        $stmt = $this->db->prepare("select * from respuesta where id_pregunta=? order by fecha desc limit ?,? ");
    ...
    public function listarRespuestasUsuario($pag = 1, $id_pregunta, $id_usuario_act)
    {
    ...
    public function contarPositivos($id_usuario){
        $stmt = $this->db->prepare("select count(*) as total from votacion where id_usuario=? and tipo_votacion=1");
        $stmt->execute(array($id_usuario));
        return $stmt->fetchColumn();
    }
    ...
    public function contarNegativos($id_usuario){
        $stmt = $this->db->prepare("select count(*) as total from votacion where id_usuario=? and tipo_votacion=0");
        $stmt->execute(array($id_usuario));
        return $stmt->fetchColumn();
    }
